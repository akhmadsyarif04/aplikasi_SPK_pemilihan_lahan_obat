                  ...
                  <section class="panel">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                              ...
                              <label class="control-label col-lg-2" for="inputSuccess">Lahan</label>
                              <div class="col-lg-10">
                                  <div class="row">
                                    <div class="col-lg-2">
                                      <input type="text" name="lahan" class="form-control" placeholder="">
                                    </div>
                                  </div>
                                </select>
                              </div>
...
if(isset($_POST["submit"])){
  ...
  $select2="SELECT bobot FROM options_kriteria WHERE id_options = $jarakPetani";
  $hasil2 = mysqli_query($con, $select2);
  $row2= mysqli_fetch_assoc($hasil2);
  $fuzzy2 = ($row2['bobot']-1)/(4-1);  
  $sql2 = "INSERT INTO pilihan_user 
          (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
          VALUES 
          ('1', '1', '$jarakPetani', '$id_lahan', '$fuzzy2')";
  $con->query($sql2);

  $select3="SELECT bobot FROM options_kriteria WHERE id_options = $jarakPasar";
  $hasil3 = mysqli_query($con, $select3);
  $row3= mysqli_fetch_assoc($hasil3);
  $fuzzy3 = ($row3['bobot']-1)/(4-1);
  $sql3 = "INSERT INTO pilihan_user 
        (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
        VALUES 
        ('1', '2', '$jarakPasar', '$id_lahan', '$fuzzy3')";
  $con->query($sql3);

  $select4="SELECT bobot FROM options_kriteria WHERE id_options = $jarakPemukiman";
  $hasil4 = mysqli_query($con, $select4);
  $row4= mysqli_fetch_assoc($hasil4);
  $fuzzy4 = ($row4['bobot']-1)/(4-1);
  $sql4 = "INSERT INTO pilihan_user 
          (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
          VALUES 
          ('1', '3', '$jarakPemukiman', '$id_lahan', '$fuzzy4')";
  $con->query($sql4);

  // insert sql kriteria Lokasi
  $select5="SELECT bobot FROM options_kriteria WHERE id_options = $luasTanah";
  $hasil5 = mysqli_query($con, $select5);
  $row5= mysqli_fetch_assoc($hasil5);
  $fuzzy5 = ($row5['bobot']-1)/(4-1);
  $sql5 = "INSERT INTO pilihan_user 
          (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
          VALUES 
          ('2', '4', '$luasTanah', '$id_lahan', '$fuzzy5')";
  $con->query($sql5);

  $select6="SELECT bobot FROM options_kriteria WHERE id_options = $ketinggianLingkungan";
  $hasil6 = mysqli_query($con, $select6);
  $row6= mysqli_fetch_assoc($hasil6);
  $fuzzy6 = ($row6['bobot']-1)/(4-1);
  $sql6 = "INSERT INTO pilihan_user 
        (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
        VALUES 
        ('2', '5', '$ketinggianLingkungan', '$id_lahan', '$fuzzy6')";
  $con->query($sql6);

  $select7="SELECT bobot FROM options_kriteria WHERE id_options = $jenisTanah";
  $hasil7 = mysqli_query($con, $select7);
  $row7= mysqli_fetch_assoc($hasil7);
  $fuzzy7 = ($row7['bobot']-1)/(4-1);
  $sql7 = "INSERT INTO pilihan_user 
          (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
          VALUES 
          ('2', '6', '$jenisTanah', '$id_lahan', '$fuzzy7')";
  $con->query($sql7);

  $select8="SELECT bobot FROM options_kriteria WHERE id_options = $hargaTanah";
  $hasil8 = mysqli_query($con, $select8);
  $row8= mysqli_fetch_assoc($hasil8);
  $fuzzy8 = ($row8['bobot']-1)/(4-1);
  $sql8 = "INSERT INTO pilihan_user 
            (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
            VALUES 
            ('2', '7', '$hargaTanah', '$id_lahan', '$fuzzy8')";
  $con->query($sql8);

  // insert sql kriteria iklim
  $select9="SELECT bobot FROM options_kriteria WHERE id_options = $panas";
  $hasil9 = mysqli_query($con, $select9);
  $row9= mysqli_fetch_assoc($hasil9);
  $fuzzy9 = ($row9['bobot']-1)/(4-1);
  $sql9 = "INSERT INTO pilihan_user 
        (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
        VALUES 
        ('3', '8', '$panas', '$id_lahan', '$fuzzy9')";
  $con->query($sql9);
  
  $select10="SELECT bobot FROM options_kriteria WHERE id_options = $dingin";
  $hasil10 = mysqli_query($con, $select10);
  $row10= mysqli_fetch_assoc($hasil10);
  $fuzzy10 = ($row10['bobot']-1)/(4-1);
  $sql10 = "INSERT INTO pilihan_user 
        (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
        VALUES 
        ('3', '9', '$dingin', '$id_lahan', '$fuzzy10')";
  $con->query($sql10);
  
  $select11="SELECT bobot FROM options_kriteria WHERE id_options = $curahHujan";
  $hasil11 = mysqli_query($con, $select11);
  $row11= mysqli_fetch_assoc($hasil11);
  $fuzzy11 = ($row11['bobot']-1)/(4-1);
  $sql11 = "INSERT INTO pilihan_user 
        (id_kriteria, id_sub_kriteria, id_options, id_lahan, fuzzy_user) 
        VALUES 
        ('3', '10', '$curahHujan', '$id_lahan', '$fuzzy11')";
  $con->query($sql11);
